...
### 3.1 Kafka Client 버전 업그레이드
*   **Apache Kafka 3.7+**: 가상 스레드 친화적인 구조로 개선하기 위한 작업들이 진행되고 있습니다. 최신 마이너 버전을 유지하는 것이 중요합니다.
*   **KIP-932 / KIP-940**: Kafka 커뮤니티에서는 가상 스레드에서의 Pinning 문제를 인지하고, 내부 `synchronized`를 `ReentrantLock`으로 교체하는 작업을 지속적으로 수행하고 있습니다.

### 3.2 캐리어 스레드 풀 확장 (임시 조치)
근본적인 해결은 아니지만, Pinning으로 인한 고갈을 늦추기 위해 캐리어 스레드 수(JDK 21 기준 시스템 속성)를 늘릴 수 있습니다.
```bash
-Djdk.virtualThreadScheduler.parallelism=N
```

### 3.3 Platform Thread 사용 (권장)
