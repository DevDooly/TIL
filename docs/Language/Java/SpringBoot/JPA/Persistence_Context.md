...
JPA는 엔티티 수정 시 `update()` 같은 메서드를 호출할 필요가 없습니다.

1.  트랜잭션 커밋 시점에 엔티티와 **스냅샷**(최초 조회 상태)을 비교합니다.
2.  변경된 필드가 있으면 자동으로 UPDATE SQL을 생성하여 쓰기 지연 저장소에 넣습니다.
3.  DB에 반영합니다.

```java
Member member = em.find(Member.class, "member1");
member.setName("NewName"); // 데이터만 변경

// em.update(member); // 이런 코드는 필요 없음
transaction.commit(); // 자동 반영
```
